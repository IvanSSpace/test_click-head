"use client";
import { useInCartCount, useTotalBoughtCount } from "@/store/store";
  ...
  return (
    <footer className="fixed sm:hidden bottom-0 left-0 w-full text-white bg-gray-800 shadow-top shadow-gray-400">
      <div className="bg-blue-red-gradient text-white flex flex-row justify-around items-center p-4">
          ...
          <Link className="relative bg-black p-1 rounded-lg" href="/profile"><Image src='./profile.svg' width={35} height={35} alt="cart" /><div className={`${totalBoughtCount === 0 ? 'hidden' : ''} absolute text-center bg-red-700 w-6 h-6 rounded-full top-[-10px] right-[-10px]`}>{totalBoughtCount}</div></Link>
        </div>
    </footer>
  );
};
